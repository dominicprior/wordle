-- Finding the best starting word in Wordle
...
-- positions (like the Mastermind black pegs) and matching
-- characters but in the wrong position with 'm' (a smaller 'm'
-- because it is less of a match).

-- E.g. match "cat" "bat" == ".MM"
-- E.g. match "cat" "act" == "mmM"

match :: String -> String -> String
match guess target =
    let b = black guess target
        bb = map fst b
        w = white (map fst b) (map snd b)
    in
        zipWith (\x y -> if x == '+' then 'M' else y) bb w

-- Like the black pegs in Mastermind, this function
-- returns the characters in the guess that match the
-- target as the non-letter chars, '+' and '-'.
-- E.g. black "fi" "fo" == [('+', 'i'), ('-', 'o')].
-- The non-matching characters are kept so that the white
-- function below can look for the remaining matches.

black :: String -> String -> [(Char, Char)]
black = zipWith (\ x y -> if x == y then ('+', '-') else (x, y))

-- This function returns matching characters between the guess
-- and the target as an 'm' (for match).
-- E.g. white "abc" "paq" == 'm.." because the first character,
-- the 'a' matches something in the target.

white :: String -> String -> String
white "" _ = ""
white (x:xs) ys =
    if (elem x ys)
    then 'm' : white xs (L.delete x ys)
    else '.' : white xs ys

-- Return the types and quantities of matches of a guess
-- against a whole dictionary of target words.
-- E.g. types "cat" ["foo", "bat", "mat"] ==
--         fromList [("...",1),(".MM",2)]
-- A good guess word will generate many types of matches.

types :: String -> [String] -> M.Map String Int
types guess dict = foldr (addToTypes guess) M.empty dict

-- Helper function for the types function.

addToTypes :: String -> String -> M.Map String Int -> M.Map String Int
addToTypes word dictWord histSoFar =
  let m = match word dictWord
  in
    M.insertWith (+) m 1 histSoFar

-- Return the entropy score for a guess word against the
-- dictionary of words, where the entropy is the sum of
-- the "p_i log p_i" values.  A smaller score is better.
-- E.g. score "cat" ["bit", "mat", "foo"] == 0.0, because
-- the types are all distinct, namely,
-- fromList [("...",1),("..M",1),(".MM",1)].
-- Conversely, score "eye" ["bit", "mat", "foo"] == log 3,
-- which is useless because all the types are the same:
-- fromList [("...",3)].

score :: String -> [String] -> Double
score guess dict =
    let h = types guess dict
        e = M.elems h
    in
      (sum $ map plogp e) / fromIntegral (sum e)

-- Helper entropy function.

plogp :: Int -> Double
plogp x = let d = fromIntegral x in d * log d

-- The entropy scores of all the possible guesses in
-- the Wordle word list.

scores :: [(String, Double)]
scores = map (\s -> (s, score s wordList)) wordList

-- Functions for showing the best and worst guesses.
-- (Takes a few minutes to run).

best n  = take n $ L.sortBy (O.comparing snd) scores
worst n = take n $ L.sortBy (O.comparing (negate . snd)) scores

-- The Wordle list of 2315 words.
...
-- Non-Wordle stuff: anagrams and letter frequencies.

anagrams :: M.Map String [String]
anagrams = foldr
  anagramHelper M.empty wordList

anagramHelper :: String -> M.Map String [String] -> M.Map String [String]
anagramHelper s m =
  M.insertWith (++) (L.sort s) [s] m

bestAnagrams :: Int -> [[String]]
bestAnagrams n = take n $ L.sortBy (O.comparing $ negate . length) $ M.elems anagrams

frequencies :: M.Map Char Int
frequencies = foldr frequencyHelper M.empty $ concat wordList
